...
# ③ 文ごとにスレッド並列で CKYMatcher を適用
# ④ GPU が使えれば BERT を cuda に載せる
# ⑤ tqdm で 2 重進捗バー表示
# =============================================================
...
# ---- CKYAnalyzer インスタンス（GPU に移動）----
try:
    analyzer = CKYAnalyzer(device=device)      # device 引数を受ける派生クラス想定
except TypeError:
    analyzer = CKYAnalyzer()
    # 内部に .model 属性があり、torch.nn.Module なら GPU へ
...
sentences = sent_df["sent"].unique().tolist()  # 列名に注意
...
# ④ 並列実行（ThreadPool）
# =============================================================
max_workers = min(8, os.cpu_count() or 4)  # コア数上限 8
print(f"スレッド並列実行: workers={max_workers}")

output_records = []
start = time.time()

with cf.ThreadPoolExecutor(max_workers=max_workers) as executor:
    futures = [executor.submit(process_sentence, row)
               for _, row in sent_df.iterrows()]
    for f in tqdm(cf.as_completed(futures), total=len(futures), desc="Sentences"):
        output_records.extend(f.result())

elapsed = time.time() - start
print(f"抽出処理時間: {elapsed:.1f} 秒")
